...
function processContains(item, list, callback) {
  return callback(list.filter(function(item){
    return true
  }))
}
...
function getFullNames(runners) {
  ...
  runners.forEach(function(items){
    return nameOrder.push(`${items.last_name}, ${items.first_name}`);
  });
return nameOrder;
}
...
function firstNamesAllCaps(runners) {
  let firstName = runners.map(function(items){
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  const runnersSize = runners.filter(function(item){
...
function tallyUpDonations(runners) {
  const donationsGiven = runners.reduce(function(accumulator, item){
